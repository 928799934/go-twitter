...
func rateLimitLimit(h http.Header) (int, error) {
	value := HeaderValues(RATE_LIMIT_LIMIT_HEADER_KEY, h)
	if len(value) == 0 {
		return 0, nil
	}
...
func rateLimitRemaining(h http.Header) (int, error) {
	value := HeaderValues(RATE_LIMIT_REMAINING_HEADER_KEY, h)
	if len(value) == 0 {
		return 0, nil
	}
...
func rateLimitResetAt(h http.Header) (*time.Time, error) {
	value := HeaderValues(RATE_LIMIT_RESET_HEADER_KEY, h)
	if len(value) == 0 {
		return nil, nil
	}
	resetInt, err := strconv.Atoi(value)
	if err != nil {
		return nil, err
	}
