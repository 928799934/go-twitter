...
func getOAuth2BearerTokenHeader(twitter *GoTwitter) (map[string]string, error) {
...
func getOAuth2AccessTokenHeader(twitter *GoTwitter) (map[string]string, error) {
	if twitter.clientAccessToken == "" && (twitter.clientID == "" || twitter.clientSecret == "") {
		return nil, fmt.Errorf("clientAccessToken, clientID, clientSecret cannot be empty at the same time")
	}

	header := make(map[string]string)
	// TODO 缺少 client_id 和 client_secret 兑换 clientAccessToken 的逻辑
	...
	header["Authorization"] = "Bearer " + twitter.clientAccessToken
	return header, nil
}
